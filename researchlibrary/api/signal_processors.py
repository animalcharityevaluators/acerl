...
class SignalProcessor(BaseSignalProcessor):
    ...
    def handle_save(self, sender, instance, **kwargs):
        ...
        """
        logger.info('Handle save sent by %r for %s', sender, instance)
    ...
    def handle_delete(self, sender, instance, **kwargs):
        ...
        """
        logger.info('Handle delete sent by %r for %s', sender, instance)
        using_backends = self.connection_router.for_write(instance=instance)
        for using in using_backends:
            index = self.connections[using].get_unified_index().get_index(sender)
    ...
    def handle_related(self, sender, instance, **kwargs):
        ...
        """
        using_backends = self.connection_router.for_write(instance=instance)
        logger.info('Handle related sent by %r for %s', sender, instance)
        for using in using_backends:
    ...
    def handle_m2m(self, sender, instance, action, reverse, model, **kwargs):
        logger.info('Handle m2m sent by %r for %s with %s on %s (%s)', sender, instance, action, model, reverse)
